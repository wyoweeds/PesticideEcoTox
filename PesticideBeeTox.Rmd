...
AgroTrak.ecotox <-
  ...
  mutate(BeeRex.EEC = (Volume.kg / BaseArea.ha) * 2.4 / AAC.LD50,
         ...
         EcoEff = ToxIndex.rate / Yield.tha)
...
An area-adjusted toxicity index ($TI_{area}$) was calculated by dividing $P_{vol}$ by planted area ($A_{plant}$) for each crop to account for year-to-year changes in hectares planted to corn and soybeans, then divided by the adult acute contact $LD_{50}$.

$$TI_{area} = (\frac{P_{vol} / A_{plant}}{LD_{50}})$$

**[note: explain here why both $TI_{rate}$ and $TI_{area}$ were calculated, and the relative benefits and deficiencies of each]**
...
plot_grid(tisum.rate.gg, tisum.area.gg,
          ncol = 1,
          labels = c("Rate-adjusted", "Area-adjusted"),
          hjust = -0.1)
```
...
* The summed honey be $TI_{area}$ over all years was `r round(ToxIndex.cropTotals$cropToxIndex.area[1]/ToxIndex.cropTotals$cropToxIndex.area[2], 1)`-times greater in corn compared to soybean.
  * I think the area-adjusted version is more important - the rate-adjusted will be misleading for ais applied at a high rate on very few acres.
